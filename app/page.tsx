...
import { Activity, BookOpen, Brain, Bug, ChevronRight, CloudRain, Compass, Crosshair, FlaskConical, Hammer, History, Lightbulb, Map as MapIcon, MessageCircle, Pause, Play, Search, Sparkles, Users, Wifi, WifiOff, ZoomIn, ZoomOut } from 'lucide-react';
...
import { actionLabels, aiStatusLabels, beliefLabels, categoryLabels, cs, formatHour, formatNumber, providerLabel, tileLabels, weatherLabels } from './locales/cs';
...
const TABS=[['world',MapIcon],['research',Search],['history',History],['concepts',Sparkles],['debug',Bug]] as const;
...
function TopBar({state,connected}:{state:SimulationState;connected:boolean}){return <header className="topbar">
  ...
  <div className="controls"><span className={`status ${state.ai.mode==='LIVING_MIND'?'online':''}`}>{state.ai.mode==='LIVING_MIND'?cs.controls.livingMind:cs.controls.mock}</span><button onClick={()=>control({paused:!state.paused})} aria-label={state.paused?cs.controls.resume:cs.controls.pause}>{state.paused?<Play/>:<Pause/>}</button>{([1,2,5,10] as const).map(speed=><button key={speed} onClick={()=>control({speed})} className={state.speed===speed?'active':''}>{speed}×</button>)}<span className={`status ${connected?'online':''}`}>{connected?<Wifi/>:<WifiOff/>}{connected?(state.paused?cs.controls.paused:cs.controls.running):cs.controls.reconnecting}</span></div>
</header>}
...
function Inspector({agent,state,panel,setPanel}:{agent:Agent;state:SimulationState;panel:'overview'|'history';setPanel:(panel:'overview'|'history')=>void}){
  ...
  return <aside className="inspector"><div className="panel-heading"><span>{panel==='history'?cs.inspector.lifeHistory:cs.inspector.inhabitant}</span><button onClick={()=>setPanel(panel==='history'?'overview':'history')}>{panel==='history'?cs.inspector.back:cs.inspector.life} <ChevronRight/></button></div>{panel==='history'?<LifeHistory agent={agent}/>:<>
    ...
    <Info title={cs.inspector.thoughtSummary}><p className="empty">{localized(agent.thoughtSummary)}</p></Info>
    <Info title={cs.inspector.motivations}><List rows={agent.motivations.slice().sort((a,b)=>b.strength-a.strength).slice(0,4).map(motivation=>[`${Math.round(motivation.strength*100)} %`,localized(motivation.text)])}/></Info>
    ...
    <Info title={cs.inspector.relevantMemories}><List rows={(agent.mind?.lastRelevantMemoryIds||[]).map(id=>agent.memories.find(memory=>memory.id===id)).filter(Boolean).slice(0,4).map(memory=>[cs.day(memory!.day),localized(memory!.text)])}/></Info>
    ...
    <Info title={cs.inspector.recentExperiences}><List rows={state.experiences.filter(experience=>experience.agentId===agent.id).slice(-4).reverse().map(experience=>[cs.day(Math.floor(experience.tick/240)+1),localized(experience.worldResult)])}/></Info>
    <Info title={cs.inspector.lastReflection}>{agent.reflections.length?<p className="empty">{localized(agent.reflections.at(-1)!.summary)}</p>:<Empty>{cs.inspector.noReflection}</Empty>}</Info>
    <Info title={cs.inspector.aiState}><div className="tags"><span>{state.ai.mode==='LIVING_MIND'?cs.controls.livingMind:cs.controls.mock}</span><span>{aiStatusLabels[agent.aiStatus]}</span></div></Info>
...
function ResearchOverlay({tab,state,selected}:{tab:keyof typeof cs.tabs;state:SimulationState;selected?:Agent}){
  ...
  const dominant=state.agents.map(agent=>{const counts=new Map<string,number>();for(const action of agent.actionHistory.slice(-30))counts.set(action.type,(counts.get(action.type)||0)+1);return [...counts.entries()].sort((a,b)=>b[1]-a[1])[0]?.[0]||'žádná'});const dominantGroups=new Map<string,number>();for(const action of dominant)dominantGroups.set(action,(dominantGroups.get(action)||0)+1);const homogenized=Math.max(0,...dominantGroups.values());
  const averageLatency=state.ai.totals.requestCount?Math.round(state.ai.totals.totalLatencyMs/state.ai.totals.requestCount):0;
  const debug=selected?{[cs.debug.keys.tick]:state.tick,[cs.debug.keys.selected]:selected.name,[cs.debug.keys.perceptionRadius]:12,[cs.debug.keys.currentAction]:displayAction(selected.action),[cs.debug.keys.thoughtSummary]:localized(selected.thoughtSummary),[cs.debug.keys.aiMode]:state.ai.mode==='LIVING_MIND'?cs.controls.livingMind:cs.controls.mock,[cs.debug.keys.aiProvider]:providerLabel(state.ai.provider),[cs.debug.keys.aiModel]:state.ai.model,[cs.debug.keys.aiEndpoint]:state.ai.endpoint||'—',[cs.debug.keys.aiThinkingMode]:cs.debug.thinkingModes[state.ai.thinkingMode],[cs.debug.keys.aiStatus]:aiStatusLabels[selected.aiStatus],[cs.debug.keys.aiPending]:state.ai.pending,[cs.debug.keys.aiLastLatency]:`${state.ai.lastRequestLatencyMs} ms`,[cs.debug.keys.aiRequests]:state.ai.totals.requestCount,[cs.debug.keys.aiFallbacks]:state.ai.totals.fallbackCount,[cs.debug.keys.aiAverageLatency]:state.ai.totals.requestCount?Math.round(state.ai.totals.totalLatencyMs/state.ai.totals.requestCount):0,[cs.debug.keys.retrievedMemories]:(selected.mind?.lastRelevantMemoryIds||[]).map(id=>selected.memories.find(memory=>memory.id===id)?.text).filter(Boolean).map(text=>localized(text!)),[cs.debug.keys.beliefs]:selected.beliefs.slice(-4).map(belief=>({[cs.debug.keys.beliefStatus]:beliefLabels[belief.status],[cs.debug.keys.beliefStatement]:localized(belief.statement),[cs.debug.keys.beliefEvidence]:belief.evidence.map(localized),[cs.debug.keys.beliefConfidence]:belief.confidence,[cs.debug.keys.beliefUpdated]:belief.updatedTick})),[cs.debug.keys.path]:selected.path.slice(0,8),[cs.debug.keys.experienceRecords]:state.experiences.length}:{};
  return <section className="research-overlay"><header><div><small>{cs.research.layer}</small><h1>{cs.tabs[tab]}</h1></div><p>{cs.research.note}</p></header>
    {tab==='research'&&<><div className="stat-grid"><Stat icon={Brain} label={cs.research.aiDecisions} value={state.ai.totals.requestCount}/><Stat icon={Lightbulb} label={cs.research.reflections} value={state.ai.research.reflections}/><Stat icon={Sparkles} label={cs.research.beliefChanges} value={state.ai.research.beliefChanges}/><Stat icon={Search} label={cs.research.questionsCreated} value={state.ai.research.questionsCreated}/><Stat icon={Search} label={cs.research.questionsRevisited} value={state.ai.research.questionsRevisited}/><Stat icon={Activity} label={cs.research.intentionsCompleted} value={state.ai.research.intentionsCompleted}/><Stat icon={Activity} label={cs.research.intentionsAbandoned} value={state.ai.research.intentionsAbandoned}/><Stat icon={Sparkles} label={cs.research.contradictions} value={state.ai.research.contradictions}/><Stat icon={MessageCircle} label={cs.research.knowledgeTransfers} value={state.ai.research.knowledgeTransfers}/><Stat icon={Activity} label={cs.research.fallbackDecisions} value={state.ai.totals.fallbackCount}/><Stat icon={Compass} label={cs.research.averageLatency} value={`${averageLatency} ${cs.research.milliseconds}`}/></div><div className="research-columns"><div><h2>{cs.research.perAgent}</h2>{state.agents.map(agent=>{const knownConcepts=state.concepts.filter(concept=>concept.knownBy.includes(agent.id)).length;const partners=new Set(agent.conversations.map(conversation=>conversation.withAgentId)).size;return <article className="question-row" key={agent.id}><b>{agent.name}</b><p>{agent.actionHistory.length} {cs.research.actions} · {agent.memories.length} {cs.research.memories} · {agent.beliefs.length} {cs.inspector.beliefs.toLowerCase()} · {agent.questions.length} {cs.research.questions}</p><p>{knownConcepts} {cs.research.concepts} · {agent.knownPeople.length} {cs.research.relationships} · {(agent.mind?.visitedRegions||[]).length} {cs.research.regionsShort} · {partners} {cs.research.partners}</p><small>{cs.research.currentIntention}: {localized(agent.intention)}</small></article>})}</div><div><h2>{cs.research.activeQuestions}</h2>{state.agents.flatMap(agent=>agent.questions.slice(0,1).map(question=><article className="question-row" key={`${agent.id}${question}`}><b>{agent.name}</b><p>{localized(question)}</p></article>))}</div></div></>}
    ...
    {tab==='debug'&&<div className="debug-grid"><div><h2>{cs.debug.individuality}</h2><div className="diversity"><strong>{Math.min(100,diversity)} %</strong><span>{cs.debug.variation}</span></div><p>{homogenized>=Math.ceil(state.agents.length*.8)?cs.debug.homogenized(homogenized,state.agents.length):diversity<35?cs.debug.converging:cs.debug.distinct}</p></div><pre>{JSON.stringify(debug,null,2)}</pre></div>}
  </section>
}

function Stat({icon:Icon,label,value}:{icon:typeof Users;label:string;value:number|string}){return <article><Icon/><div><strong>{typeof value==='number'?formatNumber(value):value}</strong><span>{label}</span></div></article>}
